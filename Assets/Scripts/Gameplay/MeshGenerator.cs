...
namespace Gameplay
{
    public class MeshGenerator
    {
        ...
        public MeshGenerator(Vector3Int size, Material[] materials, float seed)
        {
            _xSize = size.x;
            _ySize = size.y;
            _zSize = size.z;
        ...
        public GameObject Generate()
        {
            ...
            r.materials = new Material[6];
            for (int i = 0; i < 6; i++)
            {
                r.materials[i] = _materials[i];
            }
        ...
        private Vector3 GetRandom(Vector3 n) => new Vector3( 
            Random.Range(n.x - _seed, n.x + _seed),  
            Random.Range(n.y - _seed, n.y + _seed),  
            Random.Range(n.z - _seed, n.z + _seed));
        ...
        private void SetVertices()
        {
            ...
            for (int y = 0; y <= _ySize; y++)
            {
                ...
                {
                    _vertices[v++] = GetRandom(new Vector3(x, y, 0));
                }

                for (int z = 1; z <= _zSize; z++)
                {
                    _vertices[v++] = GetRandom(new Vector3(_xSize, y, z));
                }

                for (int x = _xSize - 1; x >= 0; x--)
                {
                    _vertices[v++] = GetRandom(new Vector3(x, y, _zSize));
                }

                for (int z = _zSize - 1; z > 0; z--)
                {
                    _vertices[v++] = GetRandom(new Vector3(0, y, z));
                }
            }
            ...
            for (int z = 1; z < _zSize; z++)
            {
                ...
                {
                    _vertices[v++] = GetRandom(new Vector3(x, _ySize, z));
                }
            }
            ...
            for (int z = 1; z < _zSize; z++)
            {
                for (int x = 1; x < _xSize; x++)
                {
                    _vertices[v++] = GetRandom(new Vector3(x, 0, z));
                }
            }
